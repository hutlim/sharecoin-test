...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Sharecoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Sharecoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Sharecoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Sharecoin 0.7 datadir."
    ;;
  3)
    echo "Detected Sharecoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Sharecoin 0.8 datadir."
    ;;
esac
